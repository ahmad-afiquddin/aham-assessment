...
const overallValue = computed(() => {
  const overall =
    portfolio.value?.investments?.reduce((totalValue, investment) => {
...
function getPercentageChange(investmentPrice: number, currentPrice: number) {
  return (
    Math.round(
      ((currentPrice - investmentPrice) / investmentPrice) * 100 * 100,
    ) / 100
  );
}
</script>
    ...
    <GridWrapper :span="6">
      <Table>
        ...
        <tr v-for="investment in portfolio.investments" :key="investment.id">
          <td>
            <nuxt-link :to="`/funds/${investment?.fund?.id}`"
              >{{ investment.fund?.title }}<TopRight
            /></nuxt-link>
          </td>
